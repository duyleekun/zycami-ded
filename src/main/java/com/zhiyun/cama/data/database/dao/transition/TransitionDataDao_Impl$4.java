...
public class TransitionDataDao_Impl$4
extends EntityDeletionOrUpdateAdapter {
    public final /* synthetic */ TransitionDataDao_Impl this$0;

    public TransitionDataDao_Impl$4(TransitionDataDao_Impl transitionDataDao_Impl, RoomDatabase roomDatabase) {
    ...
    public String createQuery() {
        return "UPDATE OR REPLACE `transition_data` SET `size` = ?,`path` = ?,`hash` = ?,`thumb` = ?,`littleThumb` = ?,`transitionType` = ?,`licensePath` = ?,`licenseName` = ?,`requiredPrime` = ?,`id` = ?,`res_id` = ?,`label` = ?,`label_tw` = ?,`label_en` = ?,`sort` = ? WHERE `id` = ?";
    }
}
