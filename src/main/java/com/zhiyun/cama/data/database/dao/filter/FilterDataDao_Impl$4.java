...
public class FilterDataDao_Impl$4
extends EntityDeletionOrUpdateAdapter {
    public final /* synthetic */ FilterDataDao_Impl this$0;

    public FilterDataDao_Impl$4(FilterDataDao_Impl filterDataDao_Impl, RoomDatabase roomDatabase) {
    ...
    public String createQuery() {
        return "UPDATE OR REPLACE `filter_data` SET `type_id` = ?,`size` = ?,`path` = ?,`thumb` = ?,`hash` = ?,`main_sort` = ?,`licensePath` = ?,`licenseName` = ?,`requiredPrime` = ?,`id` = ?,`res_id` = ?,`label` = ?,`label_tw` = ?,`label_en` = ?,`sort` = ? WHERE `id` = ?";
    }
}
