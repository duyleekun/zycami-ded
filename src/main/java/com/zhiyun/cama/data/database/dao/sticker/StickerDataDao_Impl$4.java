...
public class StickerDataDao_Impl$4
extends EntityDeletionOrUpdateAdapter {
    public final /* synthetic */ StickerDataDao_Impl this$0;

    public StickerDataDao_Impl$4(StickerDataDao_Impl stickerDataDao_Impl, RoomDatabase roomDatabase) {
    ...
    public String createQuery() {
        return "UPDATE OR REPLACE `sticker_data` SET `type_id` = ?,`size` = ?,`name` = ?,`scale` = ?,`thumb` = ?,`little_thumb` = ?,`path` = ?,`hash` = ?,`licensePath` = ?,`licenseName` = ?,`requiredPrime` = ?,`id` = ?,`res_id` = ?,`label` = ?,`label_tw` = ?,`label_en` = ?,`sort` = ? WHERE `id` = ?";
    }
}
