...
public class FilterFileDao_Impl$4
extends EntityDeletionOrUpdateAdapter {
    public final /* synthetic */ FilterFileDao_Impl this$0;

    public FilterFileDao_Impl$4(FilterFileDao_Impl filterFileDao_Impl, RoomDatabase roomDatabase) {
    ...
    public String createQuery() {
        return "UPDATE OR REPLACE `filter_file` SET `hash` = ?,`name` = ?,`category` = ?,`mime` = ?,`size` = ?,`url` = ?,`path` = ?,`download_timestamp` = ?,`ext_lic_url` = ?,`ext_lic_name` = ?,`ext_lic_path` = ? WHERE `hash` = ?";
    }
}
