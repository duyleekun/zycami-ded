...
public class FilterTypeDao_Impl$4
extends EntityDeletionOrUpdateAdapter {
    public final /* synthetic */ FilterTypeDao_Impl this$0;

    public FilterTypeDao_Impl$4(FilterTypeDao_Impl filterTypeDao_Impl, RoomDatabase roomDatabase) {
    ...
    public void bind(SupportSQLiteStatement supportSQLiteStatement, FilterType filterType) {
        ...
        l10 = filterType.getId();
        supportSQLiteStatement.bindLong(8, l10);
    }

    public String createQuery() {
        return "UPDATE OR REPLACE `filter_type` SET `thumb` = ?,`id` = ?,`res_id` = ?,`label` = ?,`label_tw` = ?,`label_en` = ?,`sort` = ? WHERE `id` = ?";
    }
}
