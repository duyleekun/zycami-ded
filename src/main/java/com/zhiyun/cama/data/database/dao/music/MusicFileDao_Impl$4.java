...
public class MusicFileDao_Impl$4
extends EntityDeletionOrUpdateAdapter {
    public final /* synthetic */ MusicFileDao_Impl this$0;

    public MusicFileDao_Impl$4(MusicFileDao_Impl musicFileDao_Impl, RoomDatabase roomDatabase) {
    ...
    public String createQuery() {
        return "UPDATE OR REPLACE `music_file` SET `hash` = ?,`name` = ?,`category` = ?,`mime` = ?,`size` = ?,`url` = ?,`path` = ?,`download_timestamp` = ?,`ext_lic_url` = ?,`ext_lic_name` = ?,`ext_lic_path` = ? WHERE `hash` = ?";
    }
}
