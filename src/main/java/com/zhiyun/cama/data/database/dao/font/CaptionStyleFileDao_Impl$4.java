...
public class CaptionStyleFileDao_Impl$4
extends EntityDeletionOrUpdateAdapter {
    public final /* synthetic */ CaptionStyleFileDao_Impl this$0;

    public CaptionStyleFileDao_Impl$4(CaptionStyleFileDao_Impl captionStyleFileDao_Impl, RoomDatabase roomDatabase) {
    ...
    public String createQuery() {
        return "UPDATE OR REPLACE `caption_style_file` SET `hash` = ?,`name` = ?,`category` = ?,`mime` = ?,`size` = ?,`url` = ?,`path` = ?,`download_timestamp` = ?,`ext_lic_url` = ?,`ext_lic_name` = ?,`ext_lic_path` = ? WHERE `hash` = ?";
    }
}
