...
public class MusicInfoDataDao_Impl$4
extends EntityDeletionOrUpdateAdapter {
    public final /* synthetic */ MusicInfoDataDao_Impl this$0;

    public MusicInfoDataDao_Impl$4(MusicInfoDataDao_Impl musicInfoDataDao_Impl, RoomDatabase roomDatabase) {
    ...
    public void bind(SupportSQLiteStatement supportSQLiteStatement, MusicInfoData musicInfoData) {
        ...
        l11 = musicInfoData.getId();
        supportSQLiteStatement.bindLong(11, l11);
    }

    public String createQuery() {
        return "UPDATE OR REPLACE `music_info_data` SET `id` = ?,`musicName` = ?,`albumType` = ?,`albumName` = ?,`musicPath` = ?,`musicUrl` = ?,`musicImage` = ?,`duration` = ?,`size` = ?,`import_timestamp` = ? WHERE `id` = ?";
    }
}
