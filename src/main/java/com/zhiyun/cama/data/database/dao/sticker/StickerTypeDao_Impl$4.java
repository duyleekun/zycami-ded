...
public class StickerTypeDao_Impl$4
extends EntityDeletionOrUpdateAdapter {
    public final /* synthetic */ StickerTypeDao_Impl this$0;

    public StickerTypeDao_Impl$4(StickerTypeDao_Impl stickerTypeDao_Impl, RoomDatabase roomDatabase) {
    ...
    public void bind(SupportSQLiteStatement supportSQLiteStatement, StickerType stickerType) {
        ...
        l11 = stickerType.getId();
        supportSQLiteStatement.bindLong(7, l11);
    }

    public String createQuery() {
        return "UPDATE OR REPLACE `sticker_type` SET `id` = ?,`res_id` = ?,`label` = ?,`label_tw` = ?,`label_en` = ?,`sort` = ? WHERE `id` = ?";
    }
}
