...
public class CaptionStyleDao_Impl$4
extends EntityDeletionOrUpdateAdapter {
    public final /* synthetic */ CaptionStyleDao_Impl this$0;

    public CaptionStyleDao_Impl$4(CaptionStyleDao_Impl captionStyleDao_Impl, RoomDatabase roomDatabase) {
    ...
    public String createQuery() {
        return "UPDATE OR REPLACE `caption_style_data` SET `size` = ?,`path` = ?,`hash` = ?,`thumb` = ?,`licensePath` = ?,`licenseName` = ?,`requiredPrime` = ?,`id` = ?,`res_id` = ?,`label` = ?,`label_tw` = ?,`label_en` = ?,`sort` = ? WHERE `id` = ?";
    }
}
